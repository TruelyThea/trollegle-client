...
class ClientBehavior extends Behavior {
    ...
    addAll() {
        ...
        this.addCommand("inject", "/-inject=CODE inject the JavaScript code into the process. `client` is a global variable. Beware: asyncronous errors could crash the process.", 0, function() {
        ...
        this.addCommand("color", "/-color=on|off (currently only affects future messages in an interface that supports colors, like the default interface)", 1, predAssign("color"));
        ...
        this.addCommand("pulses", "/-pulses displays the current pulses on the motherships", 0, function() {
            ["https://bellawhiskey.ca/trollegle", "https://centimeters.herokuapp.com/trollegle"].forEach(function(mothership) {
                axios.get(mothership + "/raw").then(res => {
                    let data = res.data;
                    let pulses = "Rooms currently pulsing on mothership " + mothership + ":\n";
                    try {
       	            ...
       	            if(data.pulses == "") pulses+= "No rooms are currently pulsing on this mothership.";		
                    this.log(pulses);
                }).catch(err => {
