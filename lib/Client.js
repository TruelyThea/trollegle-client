...
class Client {
    ...
    initiateUser(topics) {
        ...
        let config = _.extendOwn({}, this, {
            topicsArray: topics && topics.length ? topics : this.topicsArray
        });

        this.user = this.registerListeners(new Connection(config));
    ...
    inject(code) {
        try {
            (new Function("client", "_", "Client", "Connection", "Behavior", "ClientBehavior", code))(this, _, Client, Connection, Behavior, ClientBehavior);
        } catch (ex) {
            this.log(ex.message);
            this.logVerbose(ex);
        }
